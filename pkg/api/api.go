...
func (hs *HTTPServer) registerRoutes() {
	...
	r.Get("/file-upload", reqSignedIn, hs.Index)
	...
	if hs.Features.IsEnabled(featuremgmt.FlagSwaggerUi) {
	   ...
	   r.Group("/api", func(apiRoute routing.RouteRegister) {
		   // File Upload API
		   apiRoute.Post("/fileupload/upload", routing.Wrap(hs.FileUploadHandler))
		   apiRoute.Get("/fileupload/list", routing.Wrap(hs.FileListHandler))
		// user (signed in)
